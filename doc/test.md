...
| `TestExtractOutput_Basic` | 从输出中解析 `pipelinex-json` codec block 并存储到流水线元数据 |
...
| 测试函数 | 说明 |
|---------|------|
| `TestCodecBlockExtractor_ExtractJSON` | 从 `pipelinex-json` 代码块提取 JSON |
| `TestCodecBlockExtractor_ExtractYAML` | 从 `pipelinex-yaml` 代码块提取 YAML |
