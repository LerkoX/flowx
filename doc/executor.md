...
执行器支持三种参数传递方式，通过 `flowx.json` 中的 `paramDelivery` 字段配置：
...
// 通过 os/exec 的 Env 字段注入
```

**Docker 执行器：**
```go
// 通过 Docker API 的 Env 参数注入容器
```

**Kubernetes 执行器：**
```go
// 通过 Pod Spec 的 env 字段注入
```
...
```bash
# 示例：
python main.py --input_path=/path/to/image.png --width=800
```
...
`flowx.json` 中的 `executor` 字段描述节点本身的执行元数据（入口、镜像、语言等），而 YAML 配置中的 `Executors` 描述运行时的执行环境配置（网络、卷挂载、资源限制等）。
...
```json
{
  ...
  "executor": {
    "type": "docker",
    "image": "myregistry/image-resizer:v1.0.0",
...
两者结合后，运行时执行器会：
1. 使用 `flowx.json` 中的 `image` 拉取镜像
2. 使用 `flowx.json` 中的 `entry` 作为执行命令
3. 使用 YAML 中的 `config` 配置网络和卷挂载
4. 根据 `paramDelivery` 将参数传递给执行命令
